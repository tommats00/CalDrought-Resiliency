...
hist_filt_function <- function(id, date){
    ...
    # 🔧 NEW: if date has 2 values, assume it's a start/end range and expand it
  if (length(date) == 2) {
    date <- seq(
      from = lubridate::ymd(paste0(date[1], "-01")),
      to   = lubridate::ymd(paste0(date[2], "-01")),
      by   = "1 month"
    ) %>% format("%Y-%m")
  }
...
hist_plot_function(1068, "total", c("2013-01", "2013-06"))
```
...
# Example function output
hist_plot_function(1068, c("agriculture", "industrial", "landscape irrigation", "recycled", "total"), c("2013-01", "2013-06"))
```
...
```{r}
hist_na_specific_function <- function(id, date){
...
hist_na_specific_function(1068, c("2013-01", "2013-05"))
```
...
actual_plot_function <- function(id, date){
  ...
  ggplot(actual_filter_function(id, date), aes(x = year_ month, y = state_standard_shortage_level)) +
